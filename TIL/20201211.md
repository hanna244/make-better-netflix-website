...
<details open>
  ...
  여기서 대화(`interactive`)는 사용자와 인터페이스가 주고 받는 상호작용을 말한다. 예를 들어 인터랙티브 요소(`interactive element`)인 `<a href>`, `<button>`, `<input>`, `<select>`, `<textarea>`는 사용자가 클릭하고, 선택하고, 입력하는 등 인터페이스와 상호작용한다. 
  ...
  [[W3C]interactive-elements](https://www.w3.org/TR/html52/interactive-elements.html#interactive-elements)
...
<details open>
  ...
  `<select>`, `<input>`등 과 같은 `form`요소는 사용자에게 선택, 입력 등의 인터랙션을 요구하는 요소이다. 그래서 `UX` 관점에서 본다면 사용자의 주목을 이끌 수 있도록 테두리를 제거하지 않는 것이 좋을 것 같다는 피드백을 받았다. 

  실무에서는 간혹 클라이언트의 요구에 의해 테두리를 없애달라는 요청이 들어온다. 그럴 때는 접근성의 문제가 있어도 어.쩔.수.없.이 제거하기도 한다. 갑의 요청이기 때문에...😫
</details>
...
## 기록하기
`Footer` 스타일링 작업 중 겪은 시행착오를 기록합니다. 
...
브라우저에서 네이티브로 지원되는 모양들을 제거하거나, 다른 유형의 스타일을 적용하는 기능을 하는 속성이다. 
...
`none`으로 값을 지정하는 것 말고 `button`이나 `searchfield` 등 다른 값은 어떤 기능을 하는지 잘 모르겠었다. 강사님과 같이 살펴보니 해당 값을 넣어 줬을 때 그 값처럼 기능을 하게 해주는 것이었다. 예를 들어 `button`요소는 아니지만 `appearance: button`을 사용하면 `button` 처럼 보이게 한다. 

하지만 위의 기능을 많이 사용하지 않는 이유는 이미 `html`에는 `<button>`요소가 있기도 하고 일관성이 무너지게 된다. 

즉, `appearance`속성에서 제거 기능은 많이 사용하지만 스타일을 적용하는 기능은 많이 사용하지 않는다. 
